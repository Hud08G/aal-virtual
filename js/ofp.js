...
// Simbrief OFP parser — normalizes any format before parsing

const OFP_KEYS = [
  'Flight Number','Callsign','Departure','Arrival','Alternate',
  'Aircraft','Departure Date','Departure Time','Arrival Time',
  'Air Time','Block Time','Airframe','Flight Plan Summary',
...
function parseOFP(raw) {
  // Step 1: normalize — insert newline before every known key
  // Sort longest first so "Departure Date" is inserted before "Departure"
...
function buildParsed(map) {
  ...
  return {
    ...
    depIcao: extractICAO(get('departure')),
    arrIcao: extractICAO(get('arrival')),
    altIcao: extractICAO(get('alternate')),
    ac: get('aircraft'),
    airframe: get('airframe'),
    date: get('departure date'),
    depTime: parseTime(get('departure time')),
    arrTime: parseTime(get('arrival time')),
